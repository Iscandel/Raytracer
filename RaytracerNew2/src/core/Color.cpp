...
//const real CIEConstant::CIE_Y_INTEGRAL = 106.856895f;


namespace colorSpace
{
	extern const real CIE_X[CIE_SAMPLE_NUMBER];
	extern const real CIE_Y[CIE_SAMPLE_NUMBER];
	extern const real CIE_Z[CIE_SAMPLE_NUMBER];
	extern const real CIE_lambda[CIE_SAMPLE_NUMBER];
}
...
void ColorN::init()
{
	for (int i = 0; i < NB_SAMPLES; i++)
	{
		priv::real lambda0 = math::interp1(priv::real(i) / priv::real(NB_SAMPLES),
			(priv::real)colorSpace::LAMBDA_START, (priv::real)colorSpace::LAMBDA_END);
		priv::real lambda1 = math::interp1(priv::real(i + 1) / priv::real(NB_SAMPLES),
			(priv::real)colorSpace::LAMBDA_START, (priv::real)colorSpace::LAMBDA_END);
		X(i) = averageSpectrumSamples(colorSpace::CIE_lambda,
			colorSpace::CIE_X, (int)colorSpace::CIE_SAMPLE_NUMBER, lambda0, lambda1);
		Y(i) = averageSpectrumSamples(colorSpace::CIE_lambda,
			colorSpace::CIE_Y, (int)colorSpace::CIE_SAMPLE_NUMBER, lambda0, lambda1);
		Z(i) = averageSpectrumSamples(colorSpace::CIE_lambda,
			colorSpace::CIE_Z, (int)colorSpace::CIE_SAMPLE_NUMBER, lambda0, lambda1);
	}
}
...
#pragma warning(default:4838)
	...
	ColorN rgbRefl2SpectWhite(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBRefl2SpectWhite, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbRefl2SpectCyan(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBRefl2SpectCyan, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbRefl2SpectMagenta(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBRefl2SpectMagenta, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbRefl2SpectYellow(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBRefl2SpectYellow, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbRefl2SpectRed(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBRefl2SpectRed, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbRefl2SpectGreen(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBRefl2SpectGreen, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbRefl2SpectBlue(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBRefl2SpectBlue, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbIllum2SpectWhite(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBIllum2SpectWhite, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbIllum2SpectCyan(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBIllum2SpectCyan, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbIllum2SpectMagenta(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBIllum2SpectMagenta, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbIllum2SpectYellow(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBIllum2SpectYellow, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbIllum2SpectRed(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBIllum2SpectRed, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbIllum2SpectGreen(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBIllum2SpectGreen, NB_RGB_2_SPECT_SAMPLES));
	ColorN rgbIllum2SpectBlue(ColorN::fromSampledSpectrum(RGB2SpectLambda, RGBIllum2SpectBlue, NB_RGB_2_SPECT_SAMPLES));
}

void ColorN::setFromRGB(priv::real _r, priv::real _g, priv::real _b)
{
	priv::real rgb[3];
	...
	if (true) {//type == SPECTRUM_REFLECTANCE) {////////////////////////////////////////////////////////////////////////////////////
		...
		if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2]) {
			...
			r += rgb[0] * colorSpace::rgbRefl2SpectWhite;
			if (rgb[1] <= rgb[2]) {
				r += (rgb[1] - rgb[0]) * colorSpace::rgbRefl2SpectCyan;
				r += (rgb[2] - rgb[1]) * colorSpace::rgbRefl2SpectBlue;
			}
			else {
				r += (rgb[2] - rgb[0]) * colorSpace::rgbRefl2SpectCyan;
				r += (rgb[1] - rgb[2]) * colorSpace::rgbRefl2SpectGreen;
			}
		}
		else if (rgb[1] <= rgb[0] && rgb[1] <= rgb[2]) {
			// Compute reflectance _SampledSpectrum_ with _rgb[1]_ as minimum
			r += rgb[1] * colorSpace::rgbRefl2SpectWhite;
			if (rgb[0] <= rgb[2]) {
				r += (rgb[0] - rgb[1]) * colorSpace::rgbRefl2SpectMagenta;
				r += (rgb[2] - rgb[0]) * colorSpace::rgbRefl2SpectBlue;
			}
			else {
				r += (rgb[2] - rgb[1]) * colorSpace::rgbRefl2SpectMagenta;
				r += (rgb[0] - rgb[2]) * colorSpace::rgbRefl2SpectRed;
			}
		}
		else {
			// Compute reflectance _SampledSpectrum_ with _rgb[2]_ as minimum
			r += rgb[2] * colorSpace::rgbRefl2SpectWhite;
			if (rgb[0] <= rgb[1]) {
				r += (rgb[0] - rgb[2]) * colorSpace::rgbRefl2SpectYellow;
				r += (rgb[1] - rgb[0]) * colorSpace::rgbRefl2SpectGreen;
			}
			else {
				r += (rgb[1] - rgb[2]) * colorSpace::rgbRefl2SpectYellow;
				r += (rgb[0] - rgb[1]) * colorSpace::rgbRefl2SpectRed;
			}
		}
		r *= .94;
	}
		...
		if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2]) {
			...
			r += rgb[0] * colorSpace::rgbIllum2SpectWhite;
			if (rgb[1] <= rgb[2]) {
				r += (rgb[1] - rgb[0]) * colorSpace::rgbIllum2SpectCyan;
				r += (rgb[2] - rgb[1]) * colorSpace::rgbIllum2SpectBlue;
			}
			else {
				r += (rgb[2] - rgb[0]) * colorSpace::rgbIllum2SpectCyan;
				r += (rgb[1] - rgb[2]) * colorSpace::rgbIllum2SpectGreen;
			}
		}
		else if (rgb[1] <= rgb[0] && rgb[1] <= rgb[2]) {
			// Compute illuminant _SampledSpectrum_ with _rgb[1]_ as minimum
			r += rgb[1] * colorSpace::rgbIllum2SpectWhite;
			if (rgb[0] <= rgb[2]) {
				r += (rgb[0] - rgb[1]) * colorSpace::rgbIllum2SpectMagenta;
				r += (rgb[2] - rgb[0]) * colorSpace::rgbIllum2SpectBlue;
			}
			else {
				r += (rgb[2] - rgb[1]) * colorSpace::rgbIllum2SpectMagenta;
				r += (rgb[0] - rgb[2]) * colorSpace::rgbIllum2SpectRed;
			}
		}
		else {
			// Compute illuminant _SampledSpectrum_ with _rgb[2]_ as minimum
			r += rgb[2] * colorSpace::rgbIllum2SpectWhite;
			if (rgb[0] <= rgb[1]) {
				r += (rgb[0] - rgb[2]) * colorSpace::rgbIllum2SpectYellow;
				r += (rgb[1] - rgb[0]) * colorSpace::rgbIllum2SpectGreen;
			}
			else {
				r += (rgb[1] - rgb[2]) * colorSpace::rgbIllum2SpectYellow;
				r += (rgb[0] - rgb[1]) * colorSpace::rgbIllum2SpectRed;
			}
		}
		r *= .86445f;
	}
...
void Color3::setFromSampledSpectrum(const std::vector<priv::real>& lambda, const std::vector<priv::real>& spectrum)
{
	priv::real xyz[3] = { 0, 0, 0 };
	for (int i = 0; i < colorSpace::CIE_SAMPLE_NUMBER; ++i) {
		priv::real val = interpolateSpectrumSamples(lambda.data(), spectrum.data(), spectrum.size(), colorSpace::CIE_lambda[i]);
		xyz[0] += val * colorSpace::CIE_X[i];
		xyz[1] += val * colorSpace::CIE_Y[i];
		xyz[2] += val * colorSpace::CIE_Z[i];
	}
	priv::real scale = priv::real(colorSpace::CIE_lambda[colorSpace::CIE_SAMPLE_NUMBER - 1] - colorSpace::CIE_lambda[0]) /
		(colorSpace::CIE_Y_INTEGRAL * colorSpace::CIE_SAMPLE_NUMBER);
	xyz[0] *= scale;
	xyz[1] *= scale;
	xyz[2] *= scale;
	priv::real rgb[3];
...
const real colorSpace::CIE_X[colorSpace::CIE_SAMPLE_NUMBER] = {
...
const real colorSpace::CIE_Y[colorSpace::CIE_SAMPLE_NUMBER] = {
...
const real colorSpace::CIE_Z[colorSpace::CIE_SAMPLE_NUMBER] = {
...
const real colorSpace::CIE_lambda[colorSpace::CIE_SAMPLE_NUMBER] = {
