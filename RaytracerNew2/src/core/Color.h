...
#define RGB_ALIGNED_SAMPLES 3 + 1
#else
...
namespace colorSpace
{
	...
	//static ColorN rgbRefl2SpectWhite;
	//static ColorN rgbRefl2SpectCyan;
	//static ColorN rgbRefl2SpectMagenta;
	//static ColorN rgbRefl2SpectYellow;
	//static ColorN rgbRefl2SpectRed;
	//static ColorN rgbRefl2SpectGreen;
	//static ColorN rgbRefl2SpectBlue;
	//static ColorN rgbIllum2SpectWhite;
	//static ColorN rgbIllum2SpectCyan;
	//static ColorN rgbIllum2SpectMagenta;
	//static ColorN rgbIllum2SpectYellow;
	//static ColorN rgbIllum2SpectRed;
	//static ColorN rgbIllum2SpectGreen;
	//static ColorN rgbIllum2SpectBlue;
};

template<int N>
struct SamplesHelper
{
	static constexpr int SAMPLES = N;
#if USE_ALIGN
	static constexpr int ALIGNED = N + 1;
#else
	static constexpr int ALIGNED = N;
#endif
};
...
protected:
	...
	static void fromBlackbodyToSpect(priv::real temperature, std::vector<priv::real>& lambdaVect, std::vector<priv::real>& spectrum)
	{
		...
		const int SAMPLE_NUMBER = colorSpace::SAMPLE_NUMBER;
		const int LAMBDA_START = colorSpace::LAMBDA_START;
		const int LAMBDA_END = colorSpace::LAMBDA_END;

		for (int i = 0; i < SAMPLE_NUMBER; i++)
		{
			priv::real l = math::interp1(priv::real(i) / priv::real(SAMPLE_NUMBER),
				(priv::real)LAMBDA_START, (priv::real)LAMBDA_END);
			...
			lambdaVect.push_back((priv::real)l);
...
public:
	...
	static Color3 fromBlackbody(priv::real temperature)
	{
		...
		fromBlackbodyToSpect(temperature, lambdaVect, spectrum);
...
public:
	explicit ColorN(priv::real val = 0.)
	{
		BaseArray::setConstant(val);
	}
	...
	ColorN(Color3& p)
	{ 
		*this = fromRGB(p(0), p(1), p(2));
	}

	template <typename Derived> ColorN &operator=(const Eigen::ArrayBase<Derived>& p) {
		this->Base::operator=(p);
		return *this;
	}
	...
	void toXYZ(priv::real _xyz[3]) const {
		...
		priv::real scale = priv::real(colorSpace::LAMBDA_END - colorSpace::LAMBDA_START) /
			priv::real(colorSpace::CIE_Y_INTEGRAL * NB_SAMPLES);
		_xyz[0] *= scale;
		_xyz[1] *= scale;
		_xyz[2] *= scale;
	}
	...
	void setFromRGB(priv::real _r, priv::real _g, priv::real _b);
	//{
	//	priv::real rgb[3];
	...
	static ColorN fromBlackbody(priv::real temperature)
	{
		std::vector<priv::real> lambdaVect;
		std::vector<priv::real> spectrum;
		fromBlackbodyToSpect(temperature, lambdaVect, spectrum);
	...
	priv::real luminance() const
	{
		priv::real res = (priv::real)0.f;
		for (int i = 0; i < NB_SAMPLES; i++)
			res += Y(i) * coeff(i);

		return res * (colorSpace::LAMBDA_END - colorSpace::LAMBDA_START) /
			priv::real(colorSpace::CIE_Y_INTEGRAL * NB_SAMPLES);
	}
	...
	static ColorN fromSampledSpectrum(const priv::real* lambda, const priv::real* spectrum, int nbSamples)
	{
		ColorN res;
		for (int i = 0; i < NB_SAMPLES; i++)
		{
			...
			priv::real lambda1 = math::interp1(priv::real(i + 1) / priv::real(NB_SAMPLES),
				(priv::real)colorSpace::LAMBDA_START, (priv::real)colorSpace::LAMBDA_END);
			priv::real value = averageSpectrumSamples(lambda, spectrum, nbSamples, lambda0, lambda1);
			res(i) = (priv::real)value;
		}
	...
	void setFromSampledSpectrum(const std::vector<priv::real>& lambda, const std::vector<priv::real>& spectrum)
	{
		for (int i = 0; i < NB_SAMPLES; i++)
		{
			...
			priv::real lambda1 = math::interp1(priv::real(i + 1) / priv::real(NB_SAMPLES),
				(priv::real)colorSpace::LAMBDA_START, (priv::real)colorSpace::LAMBDA_END);
			priv::real value = averageSpectrumSamples(lambda.data(), spectrum.data(), (int)spectrum.size(), lambda0, lambda1);
			coeffRef(i) = (priv::real)value;
		}
	}
