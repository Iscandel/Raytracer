...
PerlinNoise::PerlinNoise(real persistence, int numberOfOctaves, real _testScale)
:myPersistence(persistence)
,myNumberOfOctaves(numberOfOctaves)
...
real PerlinNoise::Noise1(int x, int y) //int
{
	int n = x + y * 57;
	n = (n << 13) ^ n;
...
real PerlinNoise::SmoothedNoise_1(int x, int y) {
...
real PerlinNoise::InterpolatedNoise_1(real x, real y)
{
	int integer_X = int(x);
...
real PerlinNoise::eval(real x, real y)//PerlinNoise_2D(real x, real y)
{
	x *= testScale;
	y *= testScale;
	real total = 0;
