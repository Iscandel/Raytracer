...
:Texture(params)
{
	...
	{
		myTypeByName[rtPriv::STRING[i]] = (TextureType)i;
	}

	real persistence = params.getReal("persistence", 0.5f);
	real numberOfOctaves = params.getInt("octaves", 4);
	myColor1 = params.getColor("color1", Color::fromRGB(0.4f, 0.2f, 0.8f));
	myColor2 = params.getColor("color2", Color::fromRGB(0.1f, 0.7f, 0.3f));
	real testScale = params.getReal("scale", 100.f);
	myNoise = PerlinNoise(persistence, numberOfOctaves, testScale);
	std::string sType = params.getString("type", rtPriv::STRING[TextureType::NORMAL_MAP]);
	myType = myTypeByName[sType];

	noise2 = PerlinNoise(0.8, 3, 100);
	//for (int i = 0; i < 256; i++) {
...
Color PerlinNoiseTexture::eval(const Point2d & uv)
{
	...
	if (myType == TextureType::BLEND)
	{
		//real coeff = PerlinNoise_2D(uv.x() * testScale, uv.y() * testScale);
		real coeff = myNoise.eval(uv.x(), uv.y());
		return coeff * myColor1 + (1.f - coeff) * myColor2;
	}
	else
	{
		real coeff = noise2.eval(uv.x(), uv.y());
		real coeff2 = myNoise.eval(uv.x(), uv.y());
		return Color(coeff, coeff2, 1.f);
	}
}
...
//{
//	real total = 0;
//	int n = myNumberOfOctaves - 1;
//
//	for (unsigned int i = 0; i < (unsigned int) n; i++)
//	{
//		real frequency = (real)std::pow(2, i);
//		real amplitude = (real)std::pow(myPersistence, i);
//
